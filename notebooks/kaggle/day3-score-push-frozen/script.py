"""SCORE PUSH (leg C): deberta-v3-large with cdeotte's ACTUAL freezing recipe.
...
WHY FREEZING: reference_reproduction/RESULTS.md records that
cdeotte/how-to-train-open-book-model-part-2 -- the notebook that actually scored
0.823761 public -- froze the embeddings and the first 18 of 24 layers, leaving
77.2M of 435.1M params trainable, with lr 2e-5 and effective batch 16. This
project has been training ALL parameters, which is far less sample-efficient and
much slower per step. A local 200-step probe at lr=2e-5, effective batch 32,
full-unfreeze (scripts/validate_lr_fix_local.py) never drove loss below ln(5)
until its compressed LR schedule had almost fully decayed, and at batch 2 on a
T4 a full-unfreeze large run only completes ~420 of its 1,226 planned steps
inside the time budget -- the same starved regime. Freezing removes the backward
pass and the AdamW state for 18 layers, so more of the budget becomes real
optimizer steps.
...
def main() -> None:
    ...
    best_dir = OUT / "deberta-v3-large-frozen-score-push-best"
