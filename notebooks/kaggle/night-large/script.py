"""OVERNIGHT RUN A: give the reader the training volume it never had.
...
WHAT CHANGES HERE:
  ...
  4. OOM-adaptive batch, probed on the LONGEST contexts in the pool. The first
     overnight attempt died here: it probed train_df.head(), whose contexts are
     short, so the probe passed at batch 4 and real training then OOM'd inside
     disentangled_attention_bias. Batches pad to their longest member, so the
     probe has to use the worst case or it measures nothing.
...
def main() -> None:
    ...
    best_dir = OUT / "deberta-v3-large-night-best"
